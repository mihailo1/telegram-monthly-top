...
4. **Reply phrases** — immediate → semantic pick from `reply-phrases.json` via local Xenova `multilingual-e5-small` (`PHRASE_PICKER=embed`, precomputed `src/data/reply-phrase-embeddings.json`); no user text → random. Deferred → `Уже был пост недавно, запостим в HH:MM`. One reply per author/topic per 60s (prefer caption/text).
